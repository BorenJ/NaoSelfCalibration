function NaoCheckVis_test(Traj, COM_Loc, FootCorner0, opt)
NaoInitial;
% f = figure;
...
%     view([122,26]);title(i)
    ...
    q_temp = Traj(:,i);
    JointAngle = JointAngleAssign(q_temp);
    [R,JointPos,FootCorner, ~] = NaoForwardKinematics(JointPos0, JointAngle, FootCorner0);
    [Com,~] = NaoCOMLoc(Mass, MassLoc0,JointPos, R);
    plot3(FootCorner0(:,1),FootCorner0(:,2),FootCorner(:,3),'r-'); hold on;
    plot3(FootCorner(:,1,2),FootCorner(:,2,2),FootCorner(:,3,2),'g-'); hold on;
    fill3(FootCorner0(:,1),FootCorner0(:,2),FootCorner0(:,3),'r'); hold on;
    fill3(FootCorner(:,1,2),FootCorner(:,2,2),FootCorner(:,3,2),'g-'); hold on;
