...
function NaoVisualizeInertial(JointPos, R, FootCorner, EndeffLoc, ConvexC,  Com, MassLoc, LinkRadians, CollisionPoint, CollisionFlag, CoordOpt, ConvOpt, COMOpt, StableOpt, CollisionOpt, ColorOpt)
...
if COMOpt == 1
    if ColorOpt == 1
        plot3(MassLoc(:,1),MassLoc(:,2),MassLoc(:,3),'ro','MarkerFaceColor','r', 'MarkerEdgeColor','none','MarkerSize',15);hold on       % Plot joint COM
    else
        plot3(MassLoc(:,1),MassLoc(:,2),MassLoc(:,3),'ro','MarkerFaceColor','b','MarkerEdgeColor','none','MarkerSize',10);hold on       % Plot joint COM
    end
    plot3(Com(1),Com(2),Com(3),'rs','MarkerFaceColor','r','MarkerSize',15);% Plot 3-D COM
